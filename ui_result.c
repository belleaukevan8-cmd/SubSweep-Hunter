...
static void ui_result_draw_callback(Canvas* canvas, void* _model) {
    ...
    canvas_draw_str(canvas, 0, 10, "Results");
    ...
    snprintf(buf, sizeof(buf), "Best: %lu.%lu MHz",
        (unsigned long)(s->best_frequency / 1000000),
        (unsigned long)((s->best_frequency % 1000000) / 100000));
...
UiResult* ui_result_alloc(void) {
    ...
    ui->state = NULL;
    ui->back_cb = NULL; ui->back_ctx = NULL;
    ui->save_cb = NULL; ui->save_ctx = NULL;
    ...
    with_view_model(ui->view, UiResultModel * m, { m->ui = ui; }, false);
